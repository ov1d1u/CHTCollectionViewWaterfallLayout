...
- (BOOL) checkForFrameCollision:(CGRect)frameToCheck atIndexPath:(NSIndexPath *)indexPath overlappingHeight:(CGFloat *)height {
    NSArray *models = [(NSObject*)self.collectionView.dataSource valueForKey:@"models"];
    if (![NSStringFromClass([[models objectAtIndex:indexPath.row] class]) isEqualToString:@"IATimestampCellModel"]) {
        return NO;
    }
    
    frameToCheck = CGRectMake(0.0, frameToCheck.origin.y, self.collectionView.frame.size.width, frameToCheck.size.height);
    for (UICollectionViewLayoutAttributes *attr in self.allItemAttributes) {
        if (CGRectIntersectsRect(frameToCheck, attr.frame)) {
            if ([NSStringFromClass([[models objectAtIndex:attr.indexPath.row] class]) isEqualToString:@"IATimestampCellModel"]) {
                CGRect frameIntersection = CGRectIntersection(frameToCheck, attr.frame);
                *height = frameIntersection.size.height;
                return YES;
            }
        }
    }
